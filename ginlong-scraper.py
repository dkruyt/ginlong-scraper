...
def do_work():
    ...
    try:
        ...
        if (
            ...
            and resultJson.get("result").get("isAccept", 0) == 1
        ):
            loginSuccess = True
    ...
    if loginSuccess:
        ...
        Inverter_Temperature = resultJson["result"]["deviceWapper"]["dataJSON"].get(
            "1df"
        )
        ...
        Generation_Last_Month = resultJson["result"]["deviceWapper"]["dataJSON"].get(
            "1ru"
        )
        Power_Grid_Total_Power = resultJson["result"]["deviceWapper"]["dataJSON"].get(
            "1bq"
        )
        Total_On_grid_Generation = resultJson["result"]["deviceWapper"]["dataJSON"].get(
            "1bu"
        )
        ...
        Total_Energy_Purchased = resultJson["result"]["deviceWapper"]["dataJSON"].get(
            "1bv"
        )
        ...
        if Daily_Energy_Used is None:
            Daily_Energy_Used = 0
        Monthly_Energy_Used = resultJson["result"]["deviceWapper"]["dataJSON"].get(
            "1cp"
        )
        ...
        Battery_Charge_Percent = resultJson["result"]["deviceWapper"]["dataJSON"].get(
            "1cv"
        )
        ...
        if mqtt.lower() == "true":
            ...
            msgs.append(
                (mqtt_topic + "DC_Voltage_PV1", float(DC_Voltage_PV1), 0, False)
            )
            msgs.append(
                (mqtt_topic + "DC_Voltage_PV2", float(DC_Voltage_PV2), 0, False)
            )
            ...
            msgs.append(
                (
                    mqtt_topic + "Inverter_Temperature",
            ...
            msgs.append(
                (mqtt_topic + "Daily_Generation", float(Daily_Generation), 0, False)
            )
            msgs.append(
                (mqtt_topic + "Monthly_Generation", float(Monthly_Generation), 0, False)
            )
            msgs.append(
                (mqtt_topic + "Annual_Generation", float(Annual_Generation), 0, False)
            )
            msgs.append((mqtt_topic + "updateDate", int(updateDate), 0, False))
            msgs.append(
                (mqtt_topic + "Total_Generation", float(Total_Generation), 0, False)
            )
            msgs.append(
                (
                    mqtt_topic + "Generation_Last_Month",
                    float(Generation_Last_Month),
                    0,
                    False,
                )
            )
            msgs.append(
                (
                    mqtt_topic + "Power_Grid_Total_Power",
                    float(Power_Grid_Total_Power),
                    0,
                    False,
                )
            )
            msgs.append(
                (
                    mqtt_topic + "Total_On_grid_Generation",
                    float(Total_On_grid_Generation),
                    0,
                    False,
                )
            )
            msgs.append(
                (
                    mqtt_topic + "Total_Energy_Purchased",
            ...
            msgs.append(
                (mqtt_topic + "Consumption_Power", float(Consumption_Power), 0, False)
            )
            msgs.append(
                (mqtt_topic + "Consumption_Energy", float(Consumption_Energy), 0, False)
            )
            msgs.append(
                (mqtt_topic + "Daily_Energy_Used", float(Daily_Energy_Used), 0, False)
            )
            msgs.append(
                (
                    mqtt_topic + "Monthly_Energy_Used",
                    float(Monthly_Energy_Used),
                    0,
                    False,
                )
            )
            msgs.append(
                (mqtt_topic + "Annual_Energy_Used", float(Annual_Energy_Used), 0, False)
            )
            msgs.append(
                (
                    mqtt_topic + "Battery_Charge_Percent",
